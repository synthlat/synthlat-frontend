...
export default function WelcomeModulePage() {
  ...
  useEffect(() => {
    fetch(`/api/v1/modules/welcome/${guildId}`)
      .then(res => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/v1/modules/welcome/${guildId}`, {
        method: 'POST',
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold mb-2">Módulo de Bienvenida</h1>
          <p className="text-gray-400">Configura cómo Synthlat recibe a los nuevos miembros.</p>
        </div>
