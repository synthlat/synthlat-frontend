...
function shouldEncryptTokens() {
  return !!process.env.ENCRYPTION_KEY;
}

function maybeEncrypt(value) {
  if (!value) return value;
  if (!shouldEncryptTokens()) return value;
...
export async function createSession(discordUser, accessToken, refreshToken, expiresIn) {
  ...
  await sessions.insertOne({
    authToken,
    discordId: discordUser.id,
    accessToken: maybeEncrypt(accessToken),
    refreshToken: maybeEncrypt(refreshToken),
    expiresAt,
    createdAt: new Date()
  });
...
export async function getSession() {
  ...
  // Decrypt Discord tokens in-memory if encrypted at rest
  if (shouldEncryptTokens()) {
    try {
      session.accessToken = maybeDecrypt(session.accessToken);
      session.refreshToken = maybeDecrypt(session.refreshToken);
    } catch (e) {
      // If tokens can't be decrypted (key changed/corrupted), invalidate session
      await sessions.deleteOne({ authToken });
      return null;
    }
  }
